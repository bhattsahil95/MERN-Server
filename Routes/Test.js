...
router.post("/1", async (req, res) => {
    ...
    if (_.isEmpty(req.body)) {
        res.status(400).json({
            response: "You sent a blank entry! Data not valid!",
        });
        return;
    }
    ...
    if (!name) {
        res.status(400).json({
            response: "Invalid data format! 'name' is required.",
        });
        return;
    }
...
router.get("/5/:collectionName", async (req, res) => {
    ...
    if (!validCollections.includes(collectionName)) {
        return res
            .status(400)
            .json({
                error: "Invalid collection name. U can choose from either tests or notes. ",
            });
    }
